...
class MapController:
    @staticmethod
    def predict_risk_dummy(
        rainfall: float, elevation: float, drainage_score: float
    ) -> tuple:
        risk_score = 0.0
    ...
    def predict_flood_risk(rainfall_intensity: float):
        ...
        for hotspot in HOTSPOTS:
            ...
            if state.model is not None:
                try:
                    features = np.array(
                        [[rainfall_intensity, elevation, drainage_score]]
                    )
                    ...
                    risk_level, probability = MapController.predict_risk_dummy(
                        rainfall_intensity, elevation, drainage_score
                    )
            else:
                risk_level, probability = MapController.predict_risk_dummy(
    ...
    def calculate_route(start: str, end: str):
        ...
        for hotspot in HOTSPOTS:
            for point in route:
                dist = (
                    (point[0] - hotspot["lat"]) ** 2 + (point[1] - hotspot["lng"]) ** 2
                ) ** 0.5 * 111
                if dist < 0.5:
                    warnings.append(
                        f"⚠️ Route passes near {hotspot['name']} (Known Flood Zone)"
                    )
                    break
    ...
    def get_ward_risks(rainfall_intensity: float):
        ...
        for hotspot in HOTSPOTS:
            ...
            if state.model is not None:
                try:
                    ...
                    risk_level, _ = MapController.predict_risk_dummy(
                        rainfall_intensity, elevation, drainage_score
                    )
            else:
                risk_level, _ = MapController.predict_risk_dummy(
        ...
        for ward in WARDS:
            ...
            for pred in predictions:
                if (
                    ward["bounds"][0][0] <= pred["lat"] <= ward["bounds"][2][0]
                    and ward["bounds"][0][1] <= pred["lng"] <= ward["bounds"][2][1]
                ):
    ...
    def check_route_risk(route_geometry):
        ...
        return {"avg_risk": float(avg_risk), "risk_segments": risk_segments}

    @staticmethod
    def get_mapbox_route(start_lat, start_lon, end_lat, end_lon, profile):
        if not GOOGLE_MAPS_API_KEY:
            raise HTTPException(
                status_code=500, detail="Google Maps API key not configured on server"
            )
        ...
        params = {
            ...
            "mode": (
                "driving"
                if profile == "driving"
                else "walking" if profile == "walking" else "bicycling"
            ),
        ...
        try:
            ...
            geometry = [[coord[1], coord[0]] for coord in decoded_coords]
            ...
            raise HTTPException(
                status_code=503, detail=f"Error contacting routing service: {str(e)}"
            )
    ...
    def get_statistics():
        ...
        risk_categories = [
            f["properties"].get("risk_category", "Unknown") for f in features
        ]
    ...
    def get_high_risk_areas(threshold: float):
        ...
        high_risk_features = [
            f
            for f in state.grid_data["features"]
        ...
        return {
            ...
            "metadata": {"threshold": threshold, "count": len(high_risk_features)},
        }
    ...
    def get_ward_detail(ward_id: int):
        ...
        ward = next(
            (
                f
                for f in state.wards_data["features"]
        ...
        return {
            "ward": ward,
            "cells": {"type": "FeatureCollection", "features": ward_cells},
            "stats": {
                "total_cells": len(ward_cells),
                "high_risk_cells": sum(
                    1 for c in ward_cells if c["properties"].get("risk_score", 0) > 0.7
                ),
            },
        }
                ...
                routes.append(
                    {
                        ...
                        "color": (
                            ...
                            if avg_risk < 0.3
                                ...
                                "#f1c40f"
                                if avg_risk < 0.5
                                else "#e67e22" if avg_risk < 0.7 else "#e74c3c"
                            )
                        ),
                    }
                )
    ...
    def geocode(query: str):
        ...
        params = {
            ...
            "bounds": "28.4,76.8|28.9,77.3",  # Delhi bounds (southwest|northeast)
        }
        ...
        try:
            ...
            if "results" not in data or len(data["results"]) == 0:
                return []
            results = []
            for result in data["results"][:5]:  # Limit to 5
