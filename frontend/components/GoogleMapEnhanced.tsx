...
export default function GoogleMapEnhanced({
  ...
  const [selectedHotspot, setSelectedHotspot] =
    useState<HotspotPrediction | null>(null);
  const [selectedReport, setSelectedReport] =
    useState<CrowdsourceReport | null>(null);
  ...
  const routeSegments =
    route && route.route.length > 1
      ? route.route.slice(0, -1).map((point, idx) => {
          ...
          const color =
            riskHotspots.length > 0 ? gradientColor(nearest) : routeColor;
          return {
            path: [
