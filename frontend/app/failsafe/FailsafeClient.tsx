...
export default function WaterloggingMap() {
  ...
  useEffect(() => {
    ...
    bounds.extend({
      lat: routeData.waypoints.start.lat,
      lng: routeData.waypoints.start.lon,
    });
    bounds.extend({
      lat: routeData.waypoints.end.lat,
      lng: routeData.waypoints.end.lon,
    });
  ...
  useEffect(() => {
    return () => {
      ...
      if (gridOverlay.current) {
        gridOverlay.current.setMap(null);
        gridOverlay.current = null;
      }
      if (staticOverlay.current) {
        staticOverlay.current.setMap(null);
        staticOverlay.current = null;
      }
    };
  }, []);
                  ...
                  <div className="space-y-1.5 text-xs text-slate-700 dark:text-slate-300">
                    <div className="flex justify-between">
                      ...
                      <span className="font-medium">
                        {mapData.stats.total_cells ?? "N/A"}
                      </span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-slate-500">Avg Risk</span>
                      <span className="font-medium">
                        {mapData.stats.avg_risk != null
                          ? mapData.stats.avg_risk.toFixed(3)
                          : "N/A"}
                      </span>
                    </div>
                    <div className="flex justify-between">
                      ...
                      <span className="font-medium text-orange-600">
                        {mapData.stats.high_risk_count ?? "N/A"}
                      </span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-slate-500">Critical</span>
                      <span className="font-medium text-red-600">
                        {mapData.stats.critical_count ?? "N/A"}
                      </span>
                    </div>
                    {mapData.stats.risk_distribution && (
                  ...
                  <>
                    ...
                    <div style={{ color: "#475569" }}>
                      ...
                      <span style={{ fontWeight: 600, color: "#1e293b" }}>
                        {selectedGrid.properties.TEHSIL || "—"}
                      </span>
                    </div>
                    <div style={{ color: "#475569" }}>
                      District:{" "}
                      <span style={{ fontWeight: 600, color: "#1e293b" }}>
                        {selectedGrid.properties.DISTRICT || "—"}
                      </span>
                    </div>
                  </>
                )}
              {selectedGrid.properties.severity && (
                <>
                  ...
                  <div style={{ color: "#475569" }}>
                    Avg risk:{" "}
                    <span style={{ fontWeight: 600, color: "#1e293b" }}>
                      {selectedGrid.properties.avg_risk?.toFixed(3)}
                    </span>
                  </div>
                  <div style={{ color: "#475569" }}>
                    Cells:{" "}
                    <span style={{ fontWeight: 600, color: "#1e293b" }}>
                      {selectedGrid.properties.cell_count}
                    </span>
                  </div>
                  <div style={{ color: "#475569" }}>
                    Villages:{" "}
                    <span style={{ fontWeight: 600, color: "#1e293b" }}>
                      {selectedGrid.properties.villages || "—"}
                    </span>
                  </div>
                </>
              )}
              {selectedGrid.properties.cell_id && (
                <>
                  ...
                  <div style={{ color: "#475569" }}>
                    Risk:{" "}
                    <span style={{ fontWeight: 600, color: "#1e293b" }}>
                      {selectedGrid.properties.risk_category || "Unknown"}
                    </span>
                  </div>
                  <div style={{ color: "#475569" }}>
                    Score:
                    <span
                      style={{
                        ...
                        color: "#fff",
                        backgroundColor:
                          (selectedGrid.properties.risk_score ?? 0) > 0.7
                            ? "#e74c3c"
                            : (selectedGrid.properties.risk_score ?? 0) > 0.5
                              ? "#e67e22"
                              : (selectedGrid.properties.risk_score ?? 0) > 0.3
                                ? "#f1c40f"
                                : "#2ecc71",
                      }}
                    >
              ...
              >
                ...
                >
                  ...
                  <div style={{ color: "#475569" }}>
                    Distance:{" "}
                    <span style={{ fontWeight: 600, color: "#1e293b" }}>
                      {routeData.route.properties.distance_km.toFixed(2)} km
                    </span>
                  </div>
                  <div style={{ color: "#475569" }}>
                    Duration:{" "}
                    <span style={{ fontWeight: 600, color: "#1e293b" }}>
                      {routeData.route.properties.duration_min.toFixed(0)} min
                    </span>
                  </div>
                  <div style={{ color: "#475569" }}>
                    Risk:{" "}
                    <span
                      style={{
                        ...
                        borderRadius: 4,
                        color: "#fff",
                        backgroundColor: routeData.risk_analysis.color,
                      }}
                    >
                      {routeData.risk_analysis.risk_level}
                    </span>
                  </div>
                </div>
              </InfoWindow>
